function setupStateThree() {
    ...
    // === {genState} ===
        ...
        document.getElementById('trade-trigger-btn').addEventListener('click', function() {
            setState(11);
        });
        setupGrindSelectors();
...
function generateMachineUIHTML(dimmed = false) {
    ...
    <!-- hack-ctr -->
    <div id="trade-trigger-btn" class="connect-enabled token-trade-btn flex-core flex-row flex-center">
    <p class="nfo-p5">trade</p>
    </div>
    <!-- hack-ctr -->


    <!-- hacked-$-ctr -->
    <div class="token-ctr-mac-ui flex-core flex-row flex-center-start">
    <p class="nfo-p6">T</p>
    <p class="nfo-p7" id="gcg-tokens">${gcgTokens}</p>
    </div>
    <!-- hacked-$-ctr -->
...
function startIntervalForCounter(counterId, onAllIntervalsDone) {
    ...
    nIntervId = setInterval(counterFn, 100 );
